...
public class CreateLevel : MonoBehaviour
{
    ...
    private void generateLevel(){
        ...
        for(int i = 0; i < mapSize; i++){
            ...
            if(IsLevelValid(isDropletOnBlock[rand], displayedLevel)){
                ...
                if(isDropletOnBlock[rand].name.Contains("Platform")){ 
                    Instantiate(ground, new Vector3((i+1)*blockWidth, 0, 0), Quaternion.Euler(-90f, 0, 0f));
                }
            }
            else{
                    i--;
        ...
        Instantiate(finish, new Vector3((mapSize)*blockWidth+blockWidth, 0, 0), Quaternion.Euler(-90f, 0, 0f));
